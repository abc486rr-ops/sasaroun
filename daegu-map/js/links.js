...
export const kakaoLink = (place) =>
  `https://map.kakao.com/link/to/${enc(place.name)},${place.lat},${place.lng}`

/** appname 은 네이버 URL Scheme 의 필수 파라미터다. 호출 주체를 식별한다. */
export const naverAppLink = (place, appname = location.hostname || 'sasaroun') =>
  `nmap://route/public?dlat=${place.lat}&dlng=${place.lng}&dname=${enc(place.name)}&appname=${enc(appname)}`
