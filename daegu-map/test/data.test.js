...
test('필수 필드가 없는 장소는 제외하고 나머지는 살린다', () => {
  const r = validate(raw({
    places: [place(), place({ id: 'bad', name: '' })]
  }))
...
test('좌표가 문자열이면 제외한다', () => {
  const r = validate(raw({ places: [place({ id: 'str', lat: '35.87' })] }))
  assert.equal(r.places.length, 0)
})

test('id 가 중복되면 뒤엣것을 버린다', () => {
  const r = validate(raw({
    places: [place({ one_liner: '먼저' }), place({ one_liner: '나중' })]
  }))
  assert.equal(r.places.length, 1)
  assert.equal(r.places[0].one_liner, '먼저')
  assert.match(r.warnings.join(' '), /중복/)
})

test('없는 장소를 가리키는 참조는 떼어낸다', () => {
  const r = validate(raw({
    curators: [curator({ places: ['yurak', '없는곳'] })]
  }))
  assert.deepEqual(r.curators[0].places, ['yurak'])
  assert.match(r.warnings.join(' '), /없는곳/)
})

test('추천 장소가 없는 추천인은 목록에서 빠진다', () => {
  const r = validate(raw({
    curators: [curator(), curator({ id: 'empty', places: [] })]
  }))
  assert.deepEqual(r.curators.map(c => c.id), ['parksajang'])
})

test('참조가 전부 깨진 추천인도 빠진다', () => {
  const r = validate(raw({
    curators: [curator({ id: 'ghost', places: ['없음1', '없음2'] })]
  }))
  assert.equal(r.curators.length, 0)
})
...
test('team 추천인은 month 가 없어도 된다', () => {
  const r = validate(raw({ curators: [curator({ kind: 'team' })] }))
  assert.equal(r.curators.length, 1)
})

test('알 수 없는 kind 는 제외한다', () => {
  const r = validate(raw({ curators: [curator({ kind: 'robot' })] }))
  assert.equal(r.curators.length, 0)
})
